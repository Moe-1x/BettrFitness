    ...
    <div className="w-[calc(100%-30px)] mx-auto flex flex-col gap-5 py-8 px-8 min-h-[800px]  rounded-[32px] border-[#BCBCBC] border-2 border-solid relative">
      {(cardIndex === 1 || cardIndex === 2) && (
        <div className="absolute top-[-10px] rounded-tl-[32px] rounded-tr-[32px] left-0 w-full text-center text-white bg-primary py-2 font-bold">
          وفر %35 علي ال 3 شهور!
        </div>
      )}

      <div>
        <p className="text-primary text-nowrap max-[1299px]:text-center font-cairo font-medium text-[42px]">
          {title}
        </p>
      ...
      <div>
        <p className=" max-[1299px]:text-center text-nowrap font-cairo font-bold max-[360px]:text-[60px]  text-[66px]">
          {getPrice()}
      ...
      <div className="mt-8 flex flex-col">
        {features.map((feature, index) => (
          ...
          >
            ...
            <p className="font-cairo max-[360px]:text-[14px] font-semibold text-[16px] text-black">
              {feature}
            </p>
          </div>
        ))}
      ...
      {specialNote && (
        <div className="font-cairo font-bold text-[13px] text-white flex justify-center items-center rounded-br-[32px] rounded-tl-[10px] bg-primary absolute right-0 bottom-0 w-[230px] h-[50px]">
          {specialNote}
        </div>
      )}
    </div>
